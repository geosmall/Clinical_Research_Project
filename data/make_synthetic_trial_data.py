...
Outputs:
- Writes CSV files to `./data/csv` and README/data dictionary documentation to `./data` by default
- Use `--outdir` to write to a different folder

Command-line usage:
- `python data\\make_synthetic_trial_data.py`
- `python data\\make_synthetic_trial_data.py --seed 42`
- `python data\\make_synthetic_trial_data.py --outdir data`
"""
...
def write_csv(outdir, filename, rows):
    ...
    with open(os.path.join(csv_dir, filename), "w", newline="", encoding="utf-8") as handle:
...
def main():
    ...
    print(f"Done. Files written to ./{args.outdir} and ./{args.outdir}/csv")
