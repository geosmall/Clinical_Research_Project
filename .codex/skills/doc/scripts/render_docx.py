...
def ensure_system_tools() -> tuple[str, str]:
    missing: list[str] = []
    ...
    return resolved["soffice"], resolved["pdftoppm"]


def get_temp_root() -> str:
    preferred = os.environ.get("CODEX_DOC_TMPDIR")
    if preferred:
        makedirs(preferred, exist_ok=True)
        return preferred
    repo_tmp = join(os.getcwd(), "tmp", "docs", "_render_tmp")
    makedirs(repo_tmp, exist_ok=True)
    return repo_tmp


def make_managed_tempdir(prefix: str) -> str:
    path = join(get_temp_root(), prefix + uuid.uuid4().hex[:8])
...
def calc_dpi_via_pdf(input_path: str, max_w_px: int, max_h_px: int) -> int:
    ...
    user_profile = make_managed_tempdir("soffice_profile_")
    convert_tmp_dir = make_managed_tempdir("soffice_convert_")
    try:
        ...
        cleanup_dir(convert_tmp_dir)
        cleanup_dir(user_profile)
...
def convert_to_pdf(
    ...
    soffice = resolve_tool("soffice") or "soffice"
    user_profile_uri = Path(user_profile).resolve().as_uri()

    # Try direct DOC(X) -> PDF
    cmd_pdf = [
        soffice,
        "-env:UserInstallation=" + user_profile_uri,
        "--invisible",
        "--headless",
        "--norestore",
        "--convert-to",
