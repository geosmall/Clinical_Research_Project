...
- A SQLite database file, defaulting to `trial_workbench.db` in the repo root
- Five tables based on the current CSV dataset in [data/](D:/Codex/Clinical_Research_Project/data):
  - `sites`
  - `subjects`
  - `visits`
  - `adverse_events`
...
```powershell
.\.venv\Scripts\python.exe Dad_Stuff\sqlite_import\import_trial_csvs_to_sqlite.py
```

To write to a different SQLite file:

```powershell
.\.venv\Scripts\python.exe Dad_Stuff\sqlite_import\import_trial_csvs_to_sqlite.py --db data\trial_workbench.db
```
...
```powershell
sqlite3 trial_workbench.db ".tables"
```
